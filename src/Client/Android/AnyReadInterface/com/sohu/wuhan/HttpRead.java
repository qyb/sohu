...
public class HttpRead implements IReadable, INetwork {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.sohu.wuhan.IReadable#createArticle(java.lang.String,
	 * java.lang.String)
	 */
	@Override
	public Error createArticle(String __url) {
		...
		if (null == __url)
			return Error.ParamNull;

		try {
			content = URLEncoder.encode("access_token", encoding) + "="
					+ URLEncoder.encode(accessToken, encoding);
			content += "&" + URLEncoder.encode("url", encoding) + "="
					+ URLEncoder.encode(__url, encoding);
		} catch (Exception e) {
			return Error.Encode;
		}
	...
	public String readArticle(String key) {
		String url = domain + Constant.ARTICLE_READ + key
				+ ".xml/?access_token=" + accessToken;
	...
	public Error updateArticle(String key, Hashtable<String, String> args) {
		...
		try {
			...
			while (ee.hasMoreElements()) {
				...
				content += "&" + URLEncoder.encode(option, encoding) + "="
						+ URLEncoder.encode(args.get(option), encoding);
			}
		} catch (Exception e) {
			return Error.Encode;
		}
	...
	public boolean asyncReadCategory(Handler __handler) {
		String url = domain + Constant.CATEGORY_READ + "?access_token="
				+ accessToken;
		async.postTask(new Task(url, null, null, __handler));
		return true;
	}
	...
	public boolean asyncDeleteCategory(String __category, Handler __handler) {
		...
		try {
			content = URLEncoder.encode("access_token", encoding) + "="
					+ URLEncoder.encode(accessToken, encoding);
			content += "&" + URLEncoder.encode("category", encoding) + "="
					+ URLEncoder.encode(__category, encoding);
		} catch (Exception e) {
			return false;
		}
	...
	public boolean asyncReadImage(String key, Handler __handler) {
		String url = domain + Constant.IMAGE_READ + key + ".xml?access_token="
				+ accessToken;
		async.postTask(new Task(url, null, null, __handler));
		return true;
	}
	...
	public boolean asyncReadRawImage(String key, Handler __handler) {
		String url = domain + Constant.IMAGE_RAW_READ + key
				+ ".xml?access_token=" + accessToken;
		async.postTask(new Task(url, null, null, __handler));
		return true;
	}
